...
public class AgentSession {
	...
	public void setStatus(Presence.Mode presenceMode, int maxChats,
		...
		// extension.
		DefaultPacketExtension agentStatus = new DefaultPacketExtension(
				AgentStatus.ELEMENT_NAME, AgentStatus.NAMESPACE);
	...
	public OccupantsInfo getOccupantsInfo(String roomID) throws XMPPException {
		...
		OccupantsInfo response = (OccupantsInfo) collector
	...
	private void fireOfferRequestEvent(
		...
		Offer offer = new Offer(this.connection, this,
				requestPacket.getUserID(), requestPacket.getUserJID(),
				this.getWorkgroupJID(), new Date((new Date()).getTime()
	...
	private void fireOfferRevokeEvent(OfferRevokeProvider.OfferRevokePacket orp) {
		RevokedOffer revokedOffer = new RevokedOffer(orp.getUserJID(),
				orp.getUserID(), this.getWorkgroupJID(), orp.getSessionID(),
	...
	private void handlePacket(Packet packet) {
		if (packet instanceof OfferRequestProvider.OfferRequestPacket) {
			...
			// queue.
			QueueOverview queueOverview = (QueueOverview) presence
			...
			// queue.
			DefaultPacketExtension notifyAgents = (DefaultPacketExtension) presence
			...
			if (notifyAgents != null) {
				...
				int maxChats = Integer.parseInt(notifyAgents
						.getValue("max-chats"));
			...
			if (invite != null && workgroupJID.equals(invite.getFrom())) {
				...
				SessionID sessionIDExt = (SessionID) message.getExtension(
						SessionID.ELEMENT_NAME, SessionID.NAMESPACE);
				if (sessionIDExt != null) {
					sessionID = sessionIDExt.getSessionID();
				}

				MetaData metaDataExt = (MetaData) message.getExtension(
						MetaData.ELEMENT_NAME, MetaData.NAMESPACE);
				if (metaDataExt != null) {
					metaData = metaDataExt.getMetaData();
				}

				this.fireInvitationEvent(message.getFrom(), sessionID,
	...
	public ChatNotes getNote(String sessionID) throws XMPPException {
		...
		ChatNotes response = (ChatNotes) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
	...
	public AgentChatHistory getAgentHistory(String jid, int maxSessions,
		...
		AgentChatHistory response = (AgentChatHistory) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
	...
	public boolean hasMonitorPrivileges(Connection con) throws XMPPException {
		...
		MonitorPacket response = (MonitorPacket) collector
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
