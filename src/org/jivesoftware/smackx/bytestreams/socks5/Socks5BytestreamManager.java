...
public final class Socks5BytestreamManager implements BytestreamManager {
	...
	public Socks5BytestreamSession establishSession(String targetJID,
		...
		try {
			...
			// don't abort here, just remember the exception thrown by
			// determineProxies()
			// determineStreamHostInfos() will at least add the local Socks5
			// proxy (if enabled)
		...
		try {
			...
			Socks5Client socks5Client = new Socks5ClientForInitiator(
					usedStreamHost, digest, this.connection, sessionID,
					targetJID);
